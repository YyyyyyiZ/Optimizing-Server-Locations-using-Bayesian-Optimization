...
class ConstrainedSubmodularSolver:
    def __init__(self, n_vars, fix_num):
        self.n_vars = n_vars
        self.fix_num = fix_num
        self.gamma = np.ones(n_vars)  # 松弛参数初始化

    def solve(self, b, A, max_iter=10):
        """主求解函数：结合次模松弛与基数约束的图割方法"""
        # 分解 A = A_neg + A_pos
        ...
        for _ in range(max_iter):
            # 松弛非次模部分 A^+
            relaxed_linear = self._relax_A_pos(A_pos)

            # 构造图并求解
            x, obj = self._solve_graph_cut(b + relaxed_linear, A_neg)

            # 更新松弛参数 gamma (基于当前解 x)
            self._update_gamma(x, A_pos)
    ...
    def _relax_A_pos(self, A_pos):
        """线性松弛非次模部分 A^+：x_i x_j <= gamma_i x_i + gamma_j x_j"""
    ...
    def _solve_graph_cut(self, b, A_neg):
        ...
        # 添加节点：源点 (s)、汇点 (t)、变量节点 (u_i, v_i)
        s = g.add_vertex()
        t = g.add_vertex()
        u = g.add_vertex(n=self.n_vars)
        v = g.add_vertex(n=self.n_vars)
        ...
        for i in range(self.n_vars):
            ...
            # v_i -> t (反向边，容量为 b_i^- + LARGE_VALUE)
            e = g.add_edge(v[i], t)
            cap[e] = max(0, -b[i]) + LARGE_VALUE

        # 二次项边 (u_i -> v_j，仅处理 A^-)
        for i in range(self.n_vars):
            for j in range(i + 1, self.n_vars):
                if A_neg[i, j] != 0:
                    e = g.add_edge(u[i], v[j])
                    cap[e] = -A_neg[i, j]  # A^- <= 0，因此取负

        # 求解最小割
        res = push_relabel_max_flow(g, s, t, cap)
        cut = min_st_cut(g, s, cap, res)

        # 提取解：x_i=1 当且仅当 u_i 在源侧且 v_i 在汇侧
        x = np.zeros(self.n_vars, dtype=int)
        for i in range(self.n_vars):
            if cut[u[i]] and not cut[v[i]]:
                x[i] = 1

        # 验证基数约束
        assert sum(x) == self.fix_num, f"基数约束未满足: {sum(x)} != {self.fix_num}"

        # 计算目标值（仅用于当前松弛问题）
        obj = x @ (A_neg + A_pos) @ x + b @ x  # 注意：此处 A_pos 已被松弛
        return x, obj

    def _update_gamma(self, x, A_pos):
        """基于当前解更新松弛参数 gamma"""
        grad = np.sum(A_pos * (1 - x.reshape(-1, 1)), axis=1)  # 梯度方向
        self.gamma -= 0.1 * grad  # 简单梯度下降
        self.gamma = np.clip(self.gamma, 0, 1)  # 投影到 [0,1]

# 使用示例
n_vars = 10
fix_num = 3  # 必须选择恰好3个变量为1
b = np.random.randn(n_vars)  # 线性项
A = np.random.randn(n_vars, n_vars)
A = (A + A.T) / 2  # 对称化

solver = ConstrainedSubmodularSolver(n_vars, fix_num)
